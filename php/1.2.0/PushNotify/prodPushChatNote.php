...
$sql="SELECT UserID, AcceptChat FROM roster where (TeamID = '$teamid') AND (Inactive != '1') AND (UserID != '$myuserid') AND (Manager != '2')";
...
if($count==0){
    ...
    while ($countloop<=$totalarraycount) {
        ...
        if (strlen($row['Token'])>2) {
            ...
            $tokenAcceptNote[] = $acceptNote[$countloop]; //Keeping AcceptNote with user
    ...
    if ($numTotalTokens=="0") {
        ...
        while ($pushcount<=$arraycount) {
            ...
            if ($tokenAcceptNote[$pushcount] == "1") {
                //User accepts Team note, send
                ...
                //User does not want Team Note, Do nothing
            }
            $pushcount++;
