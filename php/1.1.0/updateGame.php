...
ob_start();
    ...
    if ($gameid == "0") { 
        $sql="INSERT INTO $tbl_name (Opponent, Park, Date, Time, HomeAway, TeamID, City, State, HomeScore, AwayScore) VALUES ('$gameopponent', '$gamelocation', '$gamedate', '$gametime', '$gamehomeaway', '$teamid', '$gamecity', '$gamestate', '$homescore', '$awayscore')";
        $result = mysqli_query($sql);
    }
    else {
        $sql="UPDATE $tbl_name SET City='$gamecity', State='$gamestate', Opponent='$gameopponent', Park='$gamelocation', Date='$gamedate', Time='$gametime', HomeAway='$gamehomeaway', HomeScore='$homescore', AwayScore='$awayscore' WHERE (GameID='$gameid')";
        $result = mysqli_query($sql);
    }
