 ...
 */
            ...
            if($count==0){
                ...
                while ($row = mysqli_fetch_assoc($result)) {
                    ...
                    $myTeams = $myTeams . "<a data-teamid='" . $row["teamid"] . "' data-leagueid='" . $row["LeagueID"] . "' data-teamname='" . htmlspecialchars($row["name"], ENT_QUOTES) . "' data-manager='" . $row["manager"] . "' data-city='" . htmlspecialchars($row["City"], ENT_QUOTES) . "' data-state='" . $row["State"] . "' data-park='" . htmlspecialchars($row["Park"], ENT_QUOTES) . "' data-sport='" . $row["SportID"] . "'";
                        if (strlen($row['LeagueID']) >= 1) {
                            $myTeams = $myTeams . "data-leagueid='" . $row['LeagueID'] . "' ";
                        }
                    $myTeams = $myTeams . "data-league='" . htmlspecialchars($row["League"], ENT_QUOTES) . "' data-teamnote='" . $row["AcceptTeamNote"] . "' data-chat='" . $row["AcceptChat"] . "' data-followers='" . $row["AcceptFollowNote"] . "' rel='external' data-ajax='false' class='createteamlink ui-link-inherit' data-transition='slide'>";
 ...
 */
            ...
            if($count==0){
                ...
                while ($row = mysqli_fetch_assoc($result)) {
                    ...
                    $archivedTeams = $archivedTeams . "<a data-teamid='" . $row["teamid"] . "' data-leagueid='" . $row["LeagueID"] . "' data-teamname='" . htmlspecialchars($row["name"], ENT_QUOTES) . "' data-manager='" . $row["manager"] . "' data-city='" . htmlspecialchars($row["City"], ENT_QUOTES) . "' data-state='" . $row["State"] . "' data-park='" . htmlspecialchars($row["Park"], ENT_QUOTES) . "' data-sport='" . $row["SportID"] . "'";
                        if (strlen($row['LeagueID']) >= 1) {
                            $archivedTeams = $archivedTeams . "data-leagueid='" . $row['LeagueID'] . "' ";
                        }
                    $archivedTeams = $archivedTeams . "data-league='" . htmlspecialchars($row["League"], ENT_QUOTES) . "' data-teamnote='" . $row["AcceptTeamNote"] . "' data-chat='" . $row["AcceptChat"] . "' data-followers='" . $row["AcceptFollowNote"] . "' rel='external' data-ajax='false' class='createteamlink ui-link-inherit' data-transition='slide'>";
